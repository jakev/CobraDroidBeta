...
static void goldfish_pdev_worker(struct work_struct *work)
{
	...
	list_for_each_entry_safe(pos, n, &pdev_bus_new_devices, list) {
		...
		if (ret)
			printk(KERN_ERR "goldfish_pdev_worker failed to register device, %s\n", pos->pdev.name);
		else
			printk(KERN_ERR "goldfish_pdev_worker registered %s\n", pos->pdev.name);
...
static void goldfish_pdev_remove(void)
{
	...
	printk(KERN_ERR "goldfish_pdev_remove could not find device at %x\n", base);
}
...
static int goldfish_new_pdev(void)
{
	...
	if (irq_count) {
		...
		dev->pdev.resource[1].start = GOLDFISH_IRQ_BASE+irq;
		dev->pdev.resource[1].end = GOLDFISH_IRQ_BASE+irq + irq_count - 1;
		dev->pdev.resource[1].flags = IORESOURCE_IRQ;
	}

	printk(KERN_INFO "goldfish_new_pdev %s at %x irq %d\n", name, base, irq);
